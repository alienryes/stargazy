...
An earlier version of this paragraph asserted the same thing from the day the real starfield landed, and it was **unfortunately inaccurate** — 1,204 stars were drawn whereas the reference site can see only about 220, so the patterns were buried in stars that are not actually visible from it. The claim is restored here only because of external observations.
...
Both default to the reference build's values (username `operations` and hostname `stargazy5.local`; the 10.1" build's script defaults to `stargazy10.local`), so `.\build5\deploy.ps1` alone works once the local ones match — otherwise pass them. Unlike `setup.sh`, this runs on a PC and talks to the Pi over SSH, so it cannot infer the remote account name. The host is resolved by mDNS, which works on either the wired or the wireless interface; use an explicit address if mDNS is unavailable.
