...
A shower is named until it stops contributing a quarter to the sporadic background; this background is shown at the foot of the page. Past that threshold it drops off, even though its activity window has not closed; a named row implies something to watch for and when a shower is at a fraction of the background rate it doesn't qualify any more. The windows vary twentyfold in how long they run past maximum, so the cut is made against the rate rather than a number of days. When nothing clears the floor the page leaves the rotation entirely.
