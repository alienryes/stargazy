...
The frame also carries an optional **addressable LED bezel ring** — 20 WS2812B LEDs (6 top, 6 bottom, 4 each side) behind a continuous slot per side, with **translucent diffuser windows printed into the frame** (2-material MMU) for an even glow; the LEDs sit 3 mm back so there are no hotspots. The riser has a **ventilation grid** over the Pi. See [`case/leds.md`](./case/leds.md) for the strip, cut plan, and wiring to the Pi.
...
| `case/case_frame_v2_windows.stl` | 4 diffuser windows (co-print) | Add as a part of the frame; assign to the **translucent** MMU extruder |
...
- LED bezel ring: 10.6 mm channel in the front bezel band, a 7 mm light slot per side with an MMU-printed 0.8 mm translucent diffuser window; LEDs seat 3 mm back; see [`case/leds.md`](./case/leds.md)
...
1. *(Optional LED ring)* Drop each LED strip segment into its bezel channel from the back (it seats 3 mm behind the printed-in diffuser window) and link the segments at the corners — see [`case/leds.md`](./case/leds.md).
...
case/                   3D-printed desktop stand (CadQuery)
  ...
  case_frame_v2_windows.stl  Diffuser windows, MMU translucent co-print
