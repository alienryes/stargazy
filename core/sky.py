...
#
# NOT A CONFIG KEY, DECIDED 2026-08-13, and the reason is not the one that keeps
# CORE_BOOST out. METEOR_COMPRESSION *is* one (`display.meteor_compression`,
# documented in both config examples), so the asymmetry is deliberate rather
# than an oversight. Nothing about this constant makes it a trap: it is a
# standalone multiplier on a rate, and a wrong value looks like slow or fast
# cloud rather than like a rendering defect.
#
# ⇒ THE TWO TIME-LAPSE RATES ARE DELIBERATELY INDEPENDENT. The panel runs cloud
# at 8x and meteors at 20x. Neither is "the" compression - each was tuned for
# legibility on its own terms and they only share a name - so setting them equal
# would make the sky read WORSE, not more coherent. The pair looks like an
# oversight and is not: do not unify them, and do not expose one half of them.
#
